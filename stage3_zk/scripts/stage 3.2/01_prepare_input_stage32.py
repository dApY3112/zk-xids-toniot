#!/usr/bin/env python
"""
Stage 3.2: Prepare input with SHIFTED w,b for semantic groups
"""
...
OUTPUT_DIR = os.path.join(STAGE3_ZK_DIR, "circuits", "semantic_groups", "build")
...
def compute_semantic_groups(w_int, x_int, group_map):
    ...
    Returns:
        G: list of 5 group contributions (Protocol, App, ConnState, Ports, Traffic)
    """
    n = len(w_int)
...
def prepare_input(test_sample_id=1):
    """
    Prepare circuit input from test vector with semantic groups
    """
    ...
    circuit_input = {
        ...
        "y_hat": y_hat,            # Public
        "G": G                     # Public (semantic group contributions)
    }
    ...
    for i, (name, val) in enumerate(zip(group_names, G)):
        print(f"   G[{i+1}] {name:20s}: {val}")
