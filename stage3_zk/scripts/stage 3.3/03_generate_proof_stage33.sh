...
echo "============================================"
echo "Generating ZK Proof Stage 3.3 (Sample $SAMPLE_ID)"
echo "============================================"

cd "$(dirname "$0")/../.."

OUT="circuits/top3_explanation/build"
WASM="$OUT/top3_explanation_js/top3_explanation.wasm"
ZKEY="$OUT/top3_explanation_final.zkey"
INPUT="$OUT/input_sample_${SAMPLE_ID}.json"
WITNESS="$OUT/witness.wtns"
PROOF="outputs/proofs/proof_stage33_sample_${SAMPLE_ID}.json"
PUBLIC="outputs/proofs/public_stage33_sample_${SAMPLE_ID}.json"

# Check input exists
if [ ! -f "$INPUT" ]; then
    echo " ERROR: Input not found at $INPUT"
    echo "Run: python scripts/01_prepare_input_stage33.py $SAMPLE_ID"
    exit 1
fi
...
node "$OUT/top3_explanation_js/generate_witness.js" \
    "$WASM" \
    "$INPUT" \
    "$WITNESS"
...
echo "  - $PUBLIC"
echo ""
echo "Next: wsl bash scripts/04_verify_proof_stage33.sh $SAMPLE_ID"