...
- **No resampling applied**: Imbalance handled via `class_weight="balanced"` (LogReg) and **imbalance-aware evaluation** (specificity/FPR, MCC, threshold tuning)
	- See `reports/baseline_extended_metrics.md` (generated by `python tools/reproduce.py metrics`)
